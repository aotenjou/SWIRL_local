...
DEFAULT_PARAMETERS = {
    ...
    # Optional: restrict candidates to these single columns (set of Column)
    "allowed_single_columns": None,
}
...
class DB2AdvisAlgorithm(SelectionAlgorithm):
    def __init__(self, database_connector, parameters=None):
        ...
        self.allowed_single_columns = self.parameters.get("allowed_single_columns")
    ...
    def _calculate_best_indexes(self, workload):
        ...
        # If provided, filter out any candidate index that uses columns
        # outside of the allowed_single_columns set
        if self.allowed_single_columns is not None:
            filtered_candidates = []
            for per_query_candidates in candidates:
                kept = []
                for index_candidate in per_query_candidates:
                    if all(column in self.allowed_single_columns for column in index_candidate.columns):
                        kept.append(index_candidate)
                filtered_candidates.append(kept)
        ...
        end_time = time.time()
        self.index_selection_time = end_time - start_time
        logging.info(f"DB2Advis algorithm completed in {self.index_selection_time:.4f} seconds")
