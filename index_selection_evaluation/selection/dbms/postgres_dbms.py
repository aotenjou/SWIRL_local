...
class PostgresDatabaseConnector(DatabaseConnector):
    def __init__(self, db_name, autocommit=False, host=None, port=None):
        ...
        logging.debug("Postgres connector created: {} (host: {}, port: {})".format(db_name, host, port))

    def create_connection(self):
        if self._connection:
            self.close()
        
        # Build connection string with optional host and port
        conn_str = "dbname={}".format(self.db_name)
        if self.host:
            conn_str += " host={}".format(self.host)
        if self.port:
            conn_str += " port={}".format(self.port)
    ...
    def drop_indexes(self):
        ...
        for index in indexes:
            ...
            if index_name.endswith("_pkey"):
                logging.debug(f"Skip dropping primary key index: {index_name}")
                continue
